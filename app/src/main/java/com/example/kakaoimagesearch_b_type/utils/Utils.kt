package com.example.kakaoimagesearch_b_type.utils
...
//Utils 객체 : 앱에서 공통적으로 사용되는 유틸리티 함수들을 포함하는 싱글턴 객체
//이를 통해 여러 위치에서 재사용 가능한 메서드들을 효과적으로 관리하고 사용
object Utils {
//    주어진 타임스탬프 문자열의 형식을 변경하는 메서드
...
//    @return                 | 변경된 형식의 날짜/시간 문자열
    fun getDateFromTimestampWithFormat(
        ...
        Log.d(TAG, "getDateFromTimestampWithFormat: $date")

        val df = SimpleDateFormat(toFormatformat)
        res = df.format(date)
        return res
    }
//    Shared Preferences에 아이템을 추가하는 메서드
...
//    Shared Preferences에서 특정 URL을 키로 하는 아이템을 제거하는 메서드
//    @param context | 앱의 현재 컨텍스트
...
//    Shared Preferences에서 모든 북마크 아이템을 가져오는 메서드
//    @param context | 앱의 현재 컨텍스트
//    @return        | 북마크된 아이템의 ArrayList
    fun getPrefBookmarkItems(context: Context): ArrayList<SearchItemModel> {
        ...
        val bookmarkItems = java.util.ArrayList<SearchItemModel>()
        val gson = GsonBuilder().create()
